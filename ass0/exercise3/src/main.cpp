...
static glm::vec3 userInteractionShadow(const glm::vec3 &selectedPos,
  ...
  // RETURN the new light position such that the light towards the selectedPos
  // is orthogonal to the normal at that location
...
void imgui() {
  ...
  ImGui::Checkbox("3: Blinn-Phong Specular Lighting",
                  &blinnPhongSpecularLighting);
...
int main(int argc, char **argv) {
  ...
  window.registerKeyCallback([&](int key, int /* scancode */, int action,
    ...
    case GLFW_KEY_T: {
      if (shiftPressed)
        ...
        shadingData.toonSpecularThreshold -= 0.001f;
      std::cout << "ToonSpecular: " << shadingData.toonSpecularThreshold
                << std::endl;
      return;
    }
    case GLFW_KEY_D: {
      ...
      std::cout << "Toon Discretization levels: " << shadingData.toonDiscretize
                << std::endl;
      return;
    }
    case GLFW_KEY_R: {
  ...
  const Shader lambertShader =
      ShaderBuilder()
          ...
          .addStage(GL_FRAGMENT_SHADER,
                    RESOURCE_ROOT "shaders/lambert_frag.glsl")
          .build();
  const Shader phongShader =
      ShaderBuilder()
          .addStage(GL_VERTEX_SHADER, RESOURCE_ROOT "shaders/vertex.glsl")
          .addStage(GL_FRAGMENT_SHADER, RESOURCE_ROOT "shaders/phong_frag.glsl")
          .build();
  const Shader blinnPhongShader =
      ShaderBuilder()
          .addStage(GL_VERTEX_SHADER, RESOURCE_ROOT "shaders/vertex.glsl")
          .addStage(GL_FRAGMENT_SHADER,
                    RESOURCE_ROOT "shaders/blinn_phong_frag.glsl")
          .build();
  const Shader toonDiffuseShader =
      ShaderBuilder()
          .addStage(GL_VERTEX_SHADER, RESOURCE_ROOT "shaders/vertex.glsl")
          .addStage(GL_FRAGMENT_SHADER,
                    RESOURCE_ROOT "shaders/toon_diffuse_frag.glsl")
          .build();
  const Shader toonSpecularShader =
      ShaderBuilder()
          .addStage(GL_VERTEX_SHADER, RESOURCE_ROOT "shaders/vertex.glsl")
          .addStage(GL_FRAGMENT_SHADER,
                    RESOURCE_ROOT "shaders/toon_specular_frag.glsl")
          .build();
  const Shader xToonShader =
      ShaderBuilder()
          .addStage(GL_VERTEX_SHADER, RESOURCE_ROOT "shaders/vertex.glsl")
          .addStage(GL_FRAGMENT_SHADER, RESOURCE_ROOT "shaders/xtoon_frag.glsl")
          .build();
  ...
  int width, height,
      sourceNumChannels; // Number of channels in source image. pixels will
                         // always be the requested number of channels (3).
  stbi_uc *pixels = stbi_load(RESOURCE_ROOT "resources/toon_map.png", &width,
                              &height, &sourceNumChannels, STBI_rgb);
  ...
  while (!window.shouldClose()) {
    ...
    if (!debug) {
      ...
      glDepthFunc(GL_EQUAL); // Only draw a pixel if it's depth matches the
                             // value stored in the depth buffer.
      ...
      for (const Light &light : lights) {
        ...
        if (!renderedSomething) {
          if (diffuseLighting) {
            ...
            // light.position, light.color and shadingData.kd.
            glUniform3fv(lambertShader.getUniformLocation("lightPos"), 1,
                         glm::value_ptr(lights[selectedLightIndex].position));
            glUniform3fv(lambertShader.getUniformLocation("lightColor"), 1,
                         glm::value_ptr(lights[selectedLightIndex].color));
            glUniform3fv(lambertShader.getUniformLocation("kd"), 1,
                         glm::value_ptr(shadingData.kd));
            render(lambertShader);
          }
          if (phongSpecularLighting || blinnPhongSpecularLighting) {
            const Shader &shader =
                phongSpecularLighting ? phongShader : blinnPhongShader;
            shader.bind();
    ...
    if (!renderedSomething) {
      debugShader.bind();
      // glUniform3fv(debugShader.getUniformLocation("viewPos"), 1,
      // glm::value_ptr(cameraPos)); // viewPos.
      render(debugShader);
    }
...
// User interaction - when the user chooses a vertex, you receive its position,
// normal, its index you can use it to NOW modify all global variables, such as
// the light position, or change material properties etc.
static void userInteraction(const glm::vec3 &cameraPos,
                            const glm::vec3 &selectedPos,
                            const glm::vec3 &selectedNormal) {
